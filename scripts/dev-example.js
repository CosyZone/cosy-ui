...
class DevExampleScript {
    constructor() {
        ...
        this.exampleDir = path.join(this.projectRoot, 'packages/cosy-framework-example')
        this.frameworkDir = path.join(this.projectRoot, 'packages/cosy-framework')

        // 解析命令行参数
        this.forceRebuild = process.argv.includes('--force') || process.argv.includes('-f')
...
`)
    ...
    async run() {
        ...
        try {
            ...
            // 2. 检查框架是否已构建
            const frameworkBuilt = await this.checkPackageBuilt(this.frameworkDir, 'cosy-framework')
