...
**Key Features:**
- 🎨 Multiple style variants (default, primary, secondary, text, cta, ghost, light, github)
...
Use `variant="github"` to create links with GitHub icons, automatically displaying the GitHub icon before the link.

<LinkPackage.ComponentContainers.Github />

## Document Navigation

Use `variant="navigation"` to create link styles specifically designed for "previous" and "next" navigation in documents.
...
| `variant` | Style variant | `'default' \| 'primary' \| 'secondary' \| 'text' \| 'cta' \| 'ghost' \| 'light' \| 'navigation' \| 'github'` | `'default'` |
...
| `navigationType` | Navigation type, automatically adds corresponding icons | `'previous' \| 'next'` | - |
