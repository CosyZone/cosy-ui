...
**💡 [View Live Demo](/en/demos/headers/custom_footer)** - Experience the complete custom footer feature
...
Custom sidebar content (highest priority). When this slot is provided, it replaces the built-in Sidebar regardless of `showSidebar` or `sidebarConfig`, while keeping the sticky positioning and height constraints.

**Use Cases**:
- Fully customized navigation structure and styling
- Brand or interaction requirements that the built-in Sidebar cannot meet
- Place dashboards, actions, or other special content in the sidebar

**Example**:

```astro
<AppLayout metaConfig={{ title: "Custom Sidebar" }}>
  <div
    slot="sidebar"
  ...
  <h1 class="cosy:text-2xl cosy:font-bold cosy:p-6">Page Content</h1>
</AppLayout>
```
