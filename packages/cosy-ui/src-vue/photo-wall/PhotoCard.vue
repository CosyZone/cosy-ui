...
<template>
  <div
    :class="[cardClasses, hoverClasses, props.class, 'not-prose']"
    :style="[
      backgroundStyle,
      props.card.style === 'image'
        ? { backgroundImage: `url(${card.src})` }
        : {},
    ]">
    <!-- 图片模式 -->
    <img
      v-if="card.style === 'default' && card.src"
      :src="card.src"
      :alt="card.alt || card.title || '照片'"
    ...
    <!-- 文字模式 -->
    <div
      v-else-if="card.style === 'text'"
      class="cosy:p-4 cosy:text-center"
      :style="textStyle">
