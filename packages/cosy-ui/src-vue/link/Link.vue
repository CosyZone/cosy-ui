...
interface Props {
    ...
    rounded?: boolean;
    size?: 'sm' | 'md' | 'lg';
    variant?: 'default' | 'primary' | 'secondary' | 'text' | 'cta' | 'ghost' | 'light' | 'navigation' | 'github';
}
...
const linkClasses = computed(() => {
    const classes = [
        ...
        // 变体样式
        props.variant === 'primary' && 'cosy:text-primary cosy:hover:text-primary-focus',
        props.variant === 'secondary' && 'cosy:text-secondary cosy:hover:text-secondary-focus',
        props.variant === 'text',
        props.variant === 'cta' && 'cosy:text-accent cosy:hover:text-accent-focus cosy:font-medium',
        props.variant === 'ghost' && 'cosy:text-base-content cosy:hover:text-base-content/80',
        props.variant === 'default' && 'cosy:text-base-content cosy:hover:text-base-content/90',
        props.variant === 'light' && 'cosy:text-white cosy:hover:text-white/90 cosy:font-medium',
        props.variant === 'navigation' && 'cosy:flex cosy:items-center cosy:gap-3 cosy:p-4 cosy:bg-base-100 cosy:border cosy:border-base-300 cosy:rounded-lg cosy:shadow-sm cosy:hover:shadow-md cosy:hover:border-base-400 cosy:transition-all cosy:duration-300 cosy:ease-in-out cosy:no-underline cosy:hover:no-underline cosy:font-medium cosy:text-base-content cosy:hover:text-base-content cosy:hover:-translate-y-1 cosy:hover:scale-[1.01] cosy:backdrop-blur-sm cosy:hover:backdrop-blur-md',
        props.variant === 'github' && 'cosy:flex cosy:items-center cosy:gap-2 cosy:text-base-content cosy:hover:text-base-content/80 cosy:transition-colors cosy:duration-200 cosy:no-underline cosy:hover:no-underline cosy:font-medium',
        ...
        // 动画效果
        props.animation === 'hover-lift' && 'cosy:hover:-translate-y-0.5 cosy:transition-transform',
        props.animation === 'hover-glow' && 'cosy:hover:brightness-125 cosy:transition-[filter]',
        props.animation === 'hover-scale' && 'cosy:hover:scale-105 cosy:transition-transform',
        ...
        // 非按钮风格下的无下划线
        !props.btn && props.noUnderline && 'cosy:no-underline cosy:hover:no-underline',
