 ...
 */
export function validateContainerProps(
	rawProps: Record<string, unknown>,
): IContainerValidationResult {
	const userSetAspect = Object.prototype.hasOwnProperty.call(
		rawProps,
		"aspectRatio",
	);
	const userSetHeight =
		Object.prototype.hasOwnProperty.call(rawProps, "height") &&
		(rawProps.height as string | undefined) !== "none";
	const userSetWidth =
		Object.prototype.hasOwnProperty.call(rawProps, "width") &&
		(rawProps.width as string | undefined) !== "none";
	...
	const incomingFit =
		(rawProps.fit as "none" | "contain" | "cover" | undefined) ?? "none";
	let resolvedFit: "none" | "contain" | "cover" = incomingFit;
	if (incomingFit !== "none" && !userSetAspect) {
		resolvedFit = "none";
		messages.push(
			'参数不完整：启用了 fit，但未设置 aspectRatio，已回退为 fit="none"。',
		);
	}

	// 规则3：潜在不可见（无 height、无 fit、无 aspectRatio 且显式 padding="none"）→ 提示并建议最小高度
	const userSetPadding = Object.prototype.hasOwnProperty.call(
		rawProps,
		"padding",
	);
	...
	if (!hasFitEnabled && !hasAspect && !hasExplicitHeight && paddingNone) {
		...
		messages.push(
			'可见性风险：未设置 height/fit/aspectRatio 且 padding="none"，可能导致容器高度为 0。已建议应用最小高度。',
		);
	}
